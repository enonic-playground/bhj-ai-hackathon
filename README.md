...
M4 (complete five-level campaign: word bank, fruit, extra life, best score and pacing evidence, without audio) is implemented; see [its handoff](handoffs/M4.md) for the submitted evidence, pending Codex's review.
...
## What works today (M1–M3 accepted; M4 implemented, pending review)
...
- No human playtest and no real-device check for the M4 campaign. Difficulty progression uses D017's starting enemy-speed values, unvalidated against play; automated browser journeys demonstrate the mechanics, not that the pacing feels right.
- Automated evidence for the fruit and extra-life mechanics is a mix of real browser play and the deterministic unit/integration suite: reaching both fruit thresholds and the 10,000-point extra life through the authored maze's 200-plus dots is impractical to demonstrate live in full within a single browser journey, so `tests/fruit.test.ts` and `tests/score.test.ts` cover the threshold, ordering and persistence edges exactly, while `e2e/campaign.spec.ts` demonstrates one real fruit spawning, being collected, and the full five-level campaign with real capture, guesses and Next level actions.
- PWA install, offline support, a service worker, manifest and icons are not implemented yet; they belong to M5.
- Mobile behavior is verified with browser emulation at 360 × 640 only; touch coverage for the new result panels reuses the same overlay layout already verified for the title, pause and game-over panels rather than a dedicated new resize screenshot. Real-device touch, installation and offline checks remain M5 work.
